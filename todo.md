...
- [x] **Auth lockdown — `swimlane`, `connectivity`, `validation`, `implementation` routers** — every endpoint converted from `publicProcedure` to `protectedProcedure`, and each resolves its org and calls the shared `assertOrgAccess` helper (`server/_core/orgAccess.ts`) before reading or writing. The helper covers all three roles: platform admin (any org), partner admin (orgs in their client), and org user (their own org) — so hospital org users (clientId null) keep access to their own org. Connectivity write endpoints (`createRow`/`updateRow`/`archiveRow`) now take `organizationSlug` for scoping; all frontend callers updated. Verified callers are all behind login (intake form, dashboard, org pages); `syncConnectivityToNotion` is a direct function call from the intake save path and bypasses the procedure, so it's unaffected. Unit tests in `server/orgAccess.test.ts`.
