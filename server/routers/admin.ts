...
import { questions, questionOptions, organizations, users, clients, intakeFileAttachments, partnerTemplates, specifications, partnerTaskTemplates } from "../../drizzle/schema";
...
export const adminRouter = router({
  ...
  /**
   * Get all active task templates for the current partner (or all, for platform admins)
   */
  ...
  /**
   * Get task templates for a specific clientId (used by intake/tasks page)
   */
  getTaskTemplatesByClient: protectedProcedure
    .input(z.object({ clientId: z.number() }))
    .query(async ({ ctx, input }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({ code: "FORBIDDEN", message: "Admin access required" });
      ...
      return db.select()
        .from(partnerTaskTemplates)
        .where(and(eq(partnerTaskTemplates.clientId, input.clientId), eq(partnerTaskTemplates.isActive, 1)))
  ...
  createTaskTemplate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Partner admins can only create tasks for their own partner
  ...
  updateTaskTemplate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Load template to verify ownership
      const [existing] = await db.select()
        .from(partnerTaskTemplates)
        .where(eq(partnerTaskTemplates.id, input.id))
        .limit(1);
      ...
      // Partner admins can only update their own partner's tasks
      ...
      await db.update(partnerTaskTemplates)
        .set(updateData)
        .where(eq(partnerTaskTemplates.id, input.id));
  ...
  /**
   * Delete (soft-delete) a task template
   */
  deleteTaskTemplate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(partnerTaskTemplates)
        .set({ isActive: 0, updatedBy: ctx.user.email || null })
        .where(eq(partnerTaskTemplates.id, input.id));
