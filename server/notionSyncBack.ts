...
/**
 * Notion-only columns that exist in the Questionnaire database but are intentionally
 * NOT synced to MySQL. These are either:
 * - Auto-generated by Notion (Updated By, Last Updated From, Last Updated, Summary)
 * - Metadata for organizing the Notion database (Section, Institution Group, Status, Site, Created At)
 * - System columns (Question, Files)
 *
 * These should not trigger unmapped-column alerts since they're intentionally excluded.
 */
export const NOTION_ONLY_COLUMNS: string[] = [
  "Updated By",
  "Last Updated From",
  "Last Updated",
  "Summary",
 ...
 * (i.e. not in KNOWN_COLUMNS or NOTION_ONLY_COLUMNS). Dynamic detection means these are still *read*,
...
export function findUnmappedColumns(mapping: ColumnMapping[]): ColumnMapping[] {
  return mapping.filter(
    (m) =>
      !KNOWN_COLUMNS.some((k) => k.notionPropertyName === m.notionPropertyName) &&
      !NOTION_ONLY_COLUMNS.includes(m.notionPropertyName)
  );
}
